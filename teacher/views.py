...
@login_required(login_url='adminlogin')
def view_student_exam_camera(request, examID):
    exam = QMODEL.Course.objects.get(id=examID)
    studentImages_ = QMODEL.WebCamSnap.objects.all().filter(exam=exam)
    studentImages = {}
    for i in studentImages_:
        if i.student not in studentImages:
            studentImages[i.student] = []
        studentImages[i.student].append(i)
    return render(request,'teacher/teacher_view_camera.html',{'exam':exam, "studentImages": studentImages})